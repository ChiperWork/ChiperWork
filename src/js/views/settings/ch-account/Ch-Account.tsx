...
import AccountName from '@/views/settings/ch-account/AccountName.tsx';
import ActiveAccount from '@/views/settings/ch-account/ActiveAccount.tsx';
import ReservedAccount from '@/views/settings/ch-account/ReservedAccount.tsx';
...
export default class IrisAccount extends Component {
  ...
  render() {
    ...
    if (this.state.irisToActive) {
      ...
      view = (
        <div>
          <p>Registre um nome de usuário ChiperWork (chiperwork/username)</p>
  ...
  checkAvailabilityFromAPI = debounce(async (name) => {
    ...
    if (res.status < 500) {
      ...
      this.setState({
        newUserNameValid: false,
        invalidUsernameMessage: 'Erro ao verificar a disponibilidade do nome de usuário',
      });
    }
  }, 500);
  ...
  async declineReserved() {
    if (!confirm(`Are you sure you want to decline chiperwork/${name}?`)) {
      return;
    }
    const pubkey = Key.getPubKey();
    const event: Partial<Event> = {
      content: `decline chiperwork/${name}`,
      kind: 1,
      tags: [],
      pubkey,
      created_at: Math.floor(Date.now() / 1000),
